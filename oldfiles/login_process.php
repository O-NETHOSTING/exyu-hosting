	...
	switch ($task)
	{
		...
		case 'resetpw':
			...
			if(query_numrows($sql) == 1)
			{
				...
				$link = "https://your-hosting.com/login_process.php?task=resetpw2&pw={$sqlsifra}&u={$row[klijentid]}&id={$md5user}";
		...
		case 'login':
			...
			if(!empty($username) && !empty($sifra))
			{
				...
				if($brojkolona == 1) 
				{
					...
					if($log['value'] == "1") { echo 'Login je privremeno iskljucen.'; die(); }
