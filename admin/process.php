...
switch (@$task)
{
	...
	case 'editmasina':
		...
		{			
			...
			mysql_query( "UPDATE box SET
				...
				`maxsrv` = '".$maxsrv."',
				`password` = '".mysql_real_escape_string($aes->encrypt($pw))."',
				`sshport` = '".$ssh2port."' WHERE `boxid` = '".$id."'" );
	...
	case 'klijent_add':
		...
		$message =
			...
			Nedavno Vam je admin registrovao nalog na <b>YOUR Hosting</b>.<br />
			<br /><br />
			Username: ".$username."<br />
			Email Address: ".$email."<br />
			Password: ".$sifra2."<br />
			Sigurnosni kod: ".$sigkod." <b>Ovaj kod nemožete menjati pa vam preporučujemo da ga zapamtite jer vam je potreban.</b><br />
			Link sajta: <a href='https://your-hosting.com'>https://your-hosting.com</a><br /><br />
				
			---------<br />
			Ne odgovarajte na ovu poruku, ovo je samo informativna poruka!<br />
			Vas <b>YOUR Hosting!</b>";
				
				
		###
		$headers  = 'MIME-Version: 1.0' . "\r\n";
		$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
		$headers .= 'From: YOUR Hosting <podrska@'.$_SERVER['SERVER_NAME'].'>' . "\r\n";
	...
	case 'emailbug':
		...
		$headers  = 'MIME-Version: 1.0' . "\r\n";
		$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
		$headers .= 'From: YOUR Hosting <info@'.$_SERVER['SERVER_NAME'].'>' . "\r\n";
		$headers .= 'X-Mailer: PHP/' . phpversion();
		#-----------------+
		$mail = mail($to, $subject, $message, $headers);
		#-----------------+
	...
	case 'izbrisi-srv':
		...
		require_once("https://your-hosting.com/includes/func.server.inc.php");
	...
	case 'klijent_edit':
		...
		$novac = mysql_real_escape_string($_POST['novac']);
		$novac = htmlspecialchars($novac);
